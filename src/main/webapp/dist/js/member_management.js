$(function() {
	...
	$("#sellerData_form").submit(function(e) {
		...
		if (select == 2) {
			if (reason.length == 0) {
				$("#verify_sellerConfirm").html(`<font color='red'>請先填寫原因!!</font>`);
				e.preventDefault();
			}
		} else if (select == "") {
			$("#verify_sellerConfirm").html(`<font color='red'>請選擇動作!!</font>`);
			e.preventDefault();
		}
	});
	...
	$("#shelterData_form").submit(function(e) {
		...
		if (select == 2) {
			if (reason.length == 0) {
				$("#verify_shelterConfirm").html(`<font color='red'>請先填寫原因!!</font>`);
				e.preventDefault();
			}
		} else if (select == "") {
			$("#verify_shelterConfirm").html(`<font color='red'>請選擇動作!!</font>`);
			e.preventDefault();
		}
	});
