...
<body class="hold-transition sidebar-mini layout-fixed">
	...
	<!-- 購買勾選商品的傳送url -->
	<form
		action="<%=request.getContextPath()%>/buylist_for_user/buylist_for_user.do"
		method="post">
							...
							<div class="cart_tabel_area table-responsive">
										...
										<c:forEach var="cart" items="${list}" varStatus="status">
											<tr id="cartItem-${cart.cartId}">
												...
												<td><input type="checkbox" name="cartIds"
													value="${cart.cartId}" class="cart-item-checkbox"
													data-price="${cart.comm.commOnsalePrice}"
													data-quantity="${cart.purchasingAmount}"
													onchange="handleCheckboxChange(this);resetAndCalculateTotalAmount(0)"
													data-seller-id="${cart.comm.seller.sellerId}"></td>
												<!--  <td><img src="../assets/img/shop/cart-1.png" alt="img"></td> -->
												<td><a
													href="<%=request.getContextPath()%>/comm_for_user/singleCommForUser.do?action=show_comm_without_customer&commId=${cart.comm.commId}">${cart.comm.commName}</a>
												</td>
												...
												<td>
													<button type="button"
														...
														class="btn btn-danger">刪除</button> <!-- 傳送purchasingAmount -->
													<input type="hidden" name="purchasingAmount_${cart.cartId}"
													value="${cart.purchasingAmount}"> <!-- 傳送sellerId -->
													<input type="hidden" name="sellerId"
													value="${cart.comm.seller.sellerId}"> <!-- 傳送couponId -->
								...
								<div class="cart_tabel_bottom">
									...
									<div class="cart_submit_btn">
										<a href="../comm_for_user/listAllCommForUser.do"
											class="btn btn_theme btn_md">Continue shopping</a>
									</div>
									<div class="cart_right_side">
										<!-- coupon btn -->
										<div class="input-group">
											<input type="text" class="form-control"
												placeholder="Your coupon code" id="coupon_code"> <input
												type="hidden" name="couponId" id="coupon_id" value="0">
											<button class="btn btn_theme btn_md" type="button"
												onclick="applyCoupon()">Enter coupon</button>
										</div>
									</div>
								</div>
							</div>
						...
						<div class="col-lg-4">
							<div class="cart_area_total_wrapper">
								<div class="cart_total_area">
									<h4 id="total_amount_display">
										Total amount: <span>$ 0</span>
									</h4>
									<h4 id="coupon_value" class="cart_voucher_amount">
										Coupon: <span>$ 0</span>
									</h4>
								</div>
								<div class="cart_total_area bg_cart_item">
									...
									<input type="hidden" name="grandAmount" id="grand_amount"
										value="">
    ...
    xhr.open('POST', '<%=request.getContextPath()%>
		...
		function updateCouponDiscount(discount) {
			document.getElementById('coupon_value').innerText = 'Coupon Discount: $'
					+ discount;
		...
		function resetAndCalculateTotalAmount(discount) {
			...
			document.getElementById('total_amount_display').innerText = 'Total Amount: $'
					+ totalAmount.toFixed(2);
			document.getElementById('grand_amount_display').innerText = 'Grand Amount: $'
